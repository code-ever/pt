...
const CavalierKing = () => {
  ...
  const [CavalierKing, setCavalierKing] = useState([])
  useEffect(() => {
    const getCavalierKing = async () => {
      try {
        const response = await axios.get(`${process.env.REACT_APP_API_URL}/api/eachCategoryView/16/`)
        setCavalierKing(response.data)
      } catch (error) {
        console.log(error)
      }
    }
