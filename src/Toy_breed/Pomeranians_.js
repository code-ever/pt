...
const Pomeranians_ = () => {
    ...
    const [Pomeranian, setPomeranian] = useState([])
    useEffect(() => {
        const etchPomeranian = async () => {
            try {
                const res = await axios.get(`${process.env.REACT_APP_API_URL}/api/eachCategoryView/19/`)
                setPomeranian(res.data)
