...
const Goldendoodle_gods = () => {
    const stotop = () => {
        window.scrollTo(0, 0);  // Scroll to top when link is clicked
    };
    const [Goldendoodle, setGoldendoodle] = useState([])
    useEffect(() => {
        const getGoldendoodle = async () => {
            try {
                const res = await axios.get(`${process.env.REACT_APP_API_URL}/api/eachCategoryView/21/`)
                setGoldendoodle(res.data)
...
export default Goldendoodle_gods