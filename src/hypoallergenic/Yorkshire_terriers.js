...
const Yorkshire_terriers = () => {
    const stotop = () => {
        window.scrollTo(0, 0);  // Scroll to top when link is clicked
    };
    const [Yorkshi, setYorkshi] = useState([])
    useEffect(() => {
        const getYorkshi = async () => {
            try {
                const res = await axios.get(`${process.env.REACT_APP_API_URL}/api/eachCategoryView/20/`)
                setYorkshi(res.data)
              ...
              {Yorkshi.map((data) => (
                  <Link to={`/details/${data.slug}`} onClick={stotop}>
                      <div className='hover:scale-105 transition-all duration-300 ease-in-out'>
                          <div data-aos='flip-down' key={data.id} className='relative shadow-lg'>
...
export default Yorkshire_terriers