...
const Australian_Shepherd = () => {

    const stotop = () => {
        window.scrollTo(0, 0);  // Scroll to top when link is clicked
    };
    const [Australian, setAustralian] = useState([])
    useEffect(() => {
        const GetAustralian = async () => {
            try {
                const res = await axios.get(`${process.env.REACT_APP_API_URL}/api/eachCategoryView/9/`)
                setAustralian(res.data)
...
export default Australian_Shepherd