...
const Golden_retroever = () => {
    const stotop = () => {
        window.scrollTo(0, 0);  // Scroll to top when link is clicked
    };
    const [Golden_retroever, setGolden_retroever] = useState([])
    useEffect(() => {
        const getGolden_retroever = async () => {
            try {
                const res = await axios.get(`${process.env.REACT_APP_API_URL}/api/eachCategoryView/6/`)
                setGolden_retroever(res.data)
...
export default Golden_retroever