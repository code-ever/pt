...
const Siberianhuskys = () => {
    const stotop = () => {
        window.scrollTo(0, 0);  // Scroll to top when link is clicked
    };

    const [Siberi, setSiberi] = useState([])
    useEffect(() => {
        const SiberiFunction = async () => {
            try {
                const response = await axios.get(`${process.env.REACT_APP_API_URL}/api/eachCategoryView/15/`)
                setSiberi(response.data)
            } catch (error) {
                console.log(error)
            }
        }
