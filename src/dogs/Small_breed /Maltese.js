...
const Maltese = () => {
    ...
    return ( 
        <div className='py-10 mt-16'>
                <div>
                <p className='text-center text-2xl md:text-4xl font-semibold capitalize mb-16'>Maltese puppies</p>
                </div>
                ...
                {maltese.map((malteseData) => (
                    <Link to={`/details/${malteseData.slug}`} onClick={stotop}>
                        <div data-aos='zoom-in-up' key={malteseData.id} className='border border-3 shadow-lg rounded-lg group hover:scale-105 transition-all duration-300'>
                            <img src={`http://localhost:8000${malteseData.images}`} alt={malteseData.petname} className='h-[260px] w-full rounded-md object-cover' />
