...
const French_bulldog = () => {
    ...
    const [French_bulldog, setFrench_bulldog] = useState([])
    useEffect(() => {
        const getFrench_bulldog = async () => {
            try {
                const response = await axios.get(`${process.env.REACT_APP_API_URL}/api/eachCategoryView/23/`)
                setFrench_bulldog(response.data)
            } catch (error) {
                console.log(error)
            }
        }
