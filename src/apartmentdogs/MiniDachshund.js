...
const MiniDachshund = () => {
  ...
  const [MiniDachshund, setMiniDachshund] = useState([])
  useEffect(() => {
    const getMiniDachshund = async () => {
      try {
        const res = await axios.get(`${process.env.REACT_APP_API_URL}/api/eachCategoryView/22/`)
        setMiniDachshund(res.data)
